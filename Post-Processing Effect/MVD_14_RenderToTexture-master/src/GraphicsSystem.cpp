...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	screen_space_geom = (int)(geometries_.size() - 1);
	
	//initialize blur shader
	screen_space_blurshader_ = new Shader("data/shaders/blur.vert", "data/shaders/blur.frag");
	Geometry ss_blur;
	ss_blur.createPlaneGeometry();
	geometries_.push_back(ss_blur);
	screen_space_blur = (int)(geometries_.size() - 1);

	//initialize blur shader
	screen_space_bloom_ = new Shader("data/shaders/bloom.vert", "data/shaders/bloom.frag");
	Geometry ss_bloom;
	ss_bloom.createPlaneGeometry();
	geometries_.push_back(ss_bloom);
	screen_space_bloom = (int)(geometries_.size() - 1);

	//init frambuffer
	frame_.initColor(window_width, window_height);
	frame2_.initColor(window_width, window_height);
...
void GraphicsSystem::update(float dt) {
  ...
  //  renderEnvironment_();
	...
	glViewport(0, 0, frame2_.width, frame2_.height);
	glBindFramebuffer(GL_FRAMEBUFFER, frame2_.framebuffer);
	glClearColor(0.0, 0.0, 0.0, 1.0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	...
	useShader(screen_space_blurshader_);
	screen_space_blurshader_->setTexture(U_SCREEN_TEXTURE, frame_.color_textures[0], 0);
	geometries_[screen_space_blur].render();
	...
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, frame2_.color_textures[0], 0);
	geometries_[screen_space_geom].render();
	glViewport(0, 0, viewport_width_, viewport_height_);
